...
export function buildHarnessInstructions(ctx: SharedContext | UserBoxContext, policy: HarnessPhasePolicy): string {
  ...
  return [
    ...
    policy.toolsAllowed
      ...
      : undefined,
    policy.toolsAllowed
      ? "HOSTING / EXPOSING SERVERS: this machine has a `host` CLI to expose a local port beyond the machine: `host <port> --public` (anyone with the URL) or `host <port> --private` (restricted). When the user asks to host, share, deploy, or publish something, DEFAULT to hosting it right here with the host CLI (public unless they say otherwise) — never suggest GitHub Pages/Netlify/Vercel or external servers first. Serve the content (e.g. `python3 -m http.server <port>` in the site directory), run BOTH the server and `host` DETACHED like any long-running process (nohup ... & disown), then give the user the URL host printed (read it from the log file) as a plain link on its own line. While hosting is active the machine intentionally stays ON (billed) so the service stays reachable; the user stops hosting from the UI header, so never stop it yourself unless asked — if asked, kill the host process and close the port (e.g. `sudo -n ufw deny <port>/tcp`)."
      : undefined,
