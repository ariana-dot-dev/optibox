...
// New proof points: billing started, hot-swap carried the partial reply, and the
// hidden context envelope was injected with tools=true on the user box.
const billing = second.find((e) => e.type === "billing.start");
const swap = second.find((e) => e.type === "handoff.swap");
...
log("proof.new_events", {
  ...
  hotSwap: Boolean(swap),
  partialCarried: swap ? Boolean((swap as any).partialShared) : false,
