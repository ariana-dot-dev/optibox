...
export function buildHarnessInstructions(ctx: SharedContext | UserBoxContext, policy: HarnessPhasePolicy): string {
  ...
  return [
    ...
    policy.toolsAllowed
      ...
      : undefined,
    policy.toolsAllowed
      ? "HOSTING / EXPOSING SERVERS: this machine has a `host` CLI to expose a local port beyond the machine: `host <port> --public` (anyone with the URL) or `host <port> --private` (restricted). Use it whenever the user wants to demo, share, or host a site/app/API. Run it DETACHED like any long-running process (nohup ... & disown), then report the URL it prints to the user (read it from the log file). While hosting is active the machine intentionally stays ON (billed) so the hosted service stays reachable; the user stops hosting from the UI header, so you never need to stop it yourself unless asked — if asked, kill the host process and close the port (e.g. `sudo -n ufw deny <port>/tcp`)."
      : undefined,
