...
.composer{position:relative;display:block;padding:12px 16px 12px;border-top:1px solid #e0e0e0;background:rgba(255,255,255,.97);backdrop-filter:blur(12px)}textarea{width:100%;min-height:58px;max-height:140px;resize:none;border:1px solid #d9d9d9;background:#fff;color:#111;padding:13px 92px 13px 13px;font:inherit;font-weight:400;outline:none;display:block}textarea:focus{border-color:#111}button{min-height:42px;border:0;background:#111;color:#fff;padding:0 16px;font:inherit;font-weight:400;cursor:pointer}button:disabled{opacity:.5;cursor:not-allowed}#send{position:absolute;right:16px;bottom:12px;height:42px;min-height:42px;background:#fc4b55;color:#fff;border:1px solid #fc4b55}
.schematic{align-self:center;border:1px solid #e0e0e0;background:#fff;padding:20px;color:#111}.schematic h2{font-family:"Funnel Display",Inter,ui-sans-serif,system-ui,sans-serif;font-size:20px;line-height:1.15;letter-spacing:-.02em;margin:0 0 18px;color:#111;font-weight:400}
.route{position:relative;display:grid;gap:0}
.node{position:relative;z-index:1;border:1px solid #e0e0e0;background:#f6f6f6;padding:12px 13px;transition:border-color .25s ease,background .25s ease,color .25s ease}.node .nodeTitle{display:block;font-size:14px;font-weight:400;color:inherit}.node .nodeSub{display:block;margin-top:3px;color:#666;font-size:11.5px;font-weight:400}
.node[data-active="1"]{border-color:#111;background:#111;color:#fff}.node[data-active="1"] .nodeSub{color:#e4e4e4}
...
.packet{position:absolute;left:0;top:0;z-index:3;font-size:16px;line-height:1;transform:translate(-50%,-50%);opacity:0;transition:top .45s cubic-bezier(.5,0,.2,1),left .45s cubic-bezier(.5,0,.2,1),opacity .25s ease;pointer-events:none;filter:drop-shadow(0 2px 5px rgba(0,0,0,.22))}
...
<main class="app">
  ...
  <footer class="composerBar" aria-label="controls">
    ...
    <button id="settingsOpen" class="iconButton" type="button" aria-label="Settings" title="Settings">⚙</button>
  </footer>
</main>
<aside class="schematic" id="schematic" data-route="idle" aria-label="backend flow diagram">
  <h2>Backend</h2>
  <div class="route" id="routeFlow">
    <div class="packet" id="packet" aria-hidden="true">✉️</div>
...
function paintDiagram(){const s=$('schematic'),packet=$('packet');if(!s||!packet||!packet.style)return;const d=diagramStage();
  ...
  packet.style.opacity=d.hidden?'0':'1';packet.textContent=d.deliver?'✅':d.error?'⚠️':'✉️';}
