...
function showWorking(localId){const c=$('chat');$('empty')?.remove();let el=workingEls.get(localId);if(!el){el=document.createElement('div');el.className='working';el.textContent='working';workingEls.set(localId,el);}c.appendChild(el);c.scrollTop=c.scrollHeight;}
function moveWorkingToBottom(){const c=$('chat');for(const el of workingEls.values())c.appendChild(el);}
function clearWorking(localId){const el=workingEls.get(localId);if(el){el.remove();workingEls.delete(localId);}}
...
function handle(ev,localId){console.debug('[trace] stream event', ev);routeEvent(ev);const t=activeTurns.get(localId);if(t&&['handoff.started','billing.start','user-box.delta','exec'].includes(ev.type)){t.boxStarted=true;t.interruptible=false;}
  // "working…" stays until the TURN is fully finished: both surfaces answered
