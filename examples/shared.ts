...
export function buildHarnessInstructions(ctx: SharedContext | UserBoxContext, policy: HarnessPhasePolicy): string {
  ...
  return [
    ...
    policy.toolsAllowed
      ...
      : undefined,
    policy.toolsAllowed
      ? "BACKGROUND PROCESSES (extremely important): when your process ends, your tool runner KILLS ITS ENTIRE PROCESS GROUP — and `nohup`/`disown`/`&` do NOT leave the process group, so servers started that way silently die the moment you finish answering (verified repeatedly). The ONLY correct way to start ANY long-running process — dev servers, APIs, watchers, tunnels — is a NEW SESSION via setsid: `mkdir -p /home/user/.logs && setsid nohup <cmd> > /home/user/.logs/<name>.log 2>&1 < /dev/null &`. Then VERIFY PERSISTENCE IN A SEPARATE, LATER TOOL CALL (never only in the same call that started it): `pgrep -f <name> && curl -s -o /dev/null -w '%{http_code}' localhost:<port>`. If that later check fails, the process did not survive — fix it before telling the user anything is running."
      : undefined,
    policy.toolsAllowed
      ? "HOSTING / EXPOSING SERVERS: this machine has a `host` CLI to expose a local port beyond the machine: `host <port> --public` (anyone with the URL) or `host <port> --private` (restricted). When you build a website or web app, HOST IT BY DEFAULT — don't wait to be asked: serve the directory (e.g. `python3 -m http.server <port>`), expose it with host (public unless the user says otherwise), and give the user the URL. Never suggest GitHub Pages/Netlify/Vercel or external servers first. CRITICAL: `host` NEVER EXITS — running it in the foreground (even as `sleep 5; host 8080 --public`) hangs your bash tool FOREVER and kills the whole turn. Start BOTH the web server and `host` in NEW SESSIONS (setsid — plain nohup/disown dies with your process group), exactly: `cd <sitedir> && setsid nohup python3 -m http.server <port> > /home/user/.logs/site.log 2>&1 < /dev/null &` then `setsid nohup host <port> --public > /home/user/.logs/host.log 2>&1 < /dev/null &` — then `sleep 2; cat /home/user/.logs/host.log` to read the URL, and in a SEPARATE later tool call verify both still run (`pgrep -f http.server; pgrep -f 'host <port>'`). If host prints an error like agent_revoked, wait 3s and retry the SAME detached command (never foreground). Give the user the URL as a plain link on its own line. While hosting is active the machine intentionally stays ON (billed) so the service stays reachable; the user stops hosting from the UI header, so never stop it yourself unless asked — if asked, kill the host process and close the port (e.g. `sudo -n ufw deny <port>/tcp`)."
      : undefined,
